...
public class VendasImpl implements VendasQueries{
	...
	public Venda buscarComItens(Long codigo) {
		...
		criteria.setResultTransformer(Criteria.DISTINCT_ROOT_ENTITY);//agrupa em venda		
	...
	public BigDecimal valorTotalNoAno() {
		Optional<BigDecimal> optional = Optional.ofNullable(
				manager.createQuery("select sum(valorTotal) from Venda where year(dataCriacao) = :ano and status = :status", BigDecimal.class) //jpql usa as propriedades da classe java não do banco de dados no banco é valor_total
	...
	public BigDecimal valorTotalNoMes() {
		Optional<BigDecimal> optional = Optional.ofNullable(
				manager.createQuery("select sum(valorTotal) from Venda where year(dataCriacao) = :ano and month(dataCriacao) = :mes and status = :status", BigDecimal.class) //jpql usa as propriedades da classe java não do banco de dados no banco é valor_total
					.setParameter("ano", Year.now().getValue())
	...
	public BigDecimal valorTicketMedioNoAno() {
		Optional<BigDecimal> optional = Optional.ofNullable(
				manager.createQuery("select sum(valorTotal)/count(*) from Venda where year(dataCriacao) = :ano and status = :status", BigDecimal.class) //jpql usa as propriedades da classe java não do banco de dados no banco é valor_total
					.setParameter("ano", Year.now().getValue())
					.setParameter("status", StatusVenda.EMITIDA)
					.getSingleResult());
		return optional.orElse(BigDecimal.ZERO);
	...
	public List<VendaMes> totalPorMes() {
		List<VendaMes> vendasMes = manager.createNamedQuery("Vendas.totalPorMes").getResultList();	//precisa colocar no JPA config o caminho do arquivo xml com a consulta
